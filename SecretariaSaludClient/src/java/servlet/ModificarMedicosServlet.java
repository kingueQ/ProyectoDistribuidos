 ...
 * @author kingu
 */
@WebServlet(name = "ModificarMedicosServlet", urlPatterns = {"/ModificarMedicosServlet"})
public class ModificarMedicosServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String medicosAcceso = "";
        if (operacion.equalsIgnoreCase("eliminar")) {
            String[] medicos = expediente[4].split("-");
            for (int i = 0; i < medicos.length; i++) {
                if (!medicos[i].equalsIgnoreCase(cedula)) {
                    medicosAcceso = medicosAcceso + medicos[i];
                }
            }
        }else{
            medicosAcceso = expediente[4] + "-" + cedula;
        }
        
        cliente = new SocketCliente(serverAddress, serverPort);
        respuesta = cliente.enviarMensaje("modificarMedicos!" + curp + "!" + medicosAcceso);
        
        HttpSession objSesion = request.getSession(true);
        objSesion.setAttribute("curp", curp);
        response.sendRedirect("miExpedienteP.jsp");
    }
